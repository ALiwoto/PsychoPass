...
dark_html_formatter = HtmlFormatter(wrapcode=True, style='rrt')
light_html_formatter = HtmlFormatter(wrapcode=True, style='default')
for dirname, _, files in os.walk('in'):
    ...
    for file in files:
        ...
        for i in soup.find_all('div', {'class': 'highlightjson'}):
            highlight_soup = BeautifulSoup(highlight(i.string, json_lexer, dark_html_formatter), 'lxml')
...
with open('out/pygments.css', 'w+') as file:
    ...
    file.write(textwrap.indent(dark_html_formatter.get_style_defs('.highlight'), '    '))
    file.write('\n}\n\n@media (prefers-color-scheme: light) {\n')
    file.write(textwrap.indent(light_html_formatter.get_style_defs('.highlight'), '    '))
    file.write('\n}')
